...
use bevy::prelude::{AnyOf, Query, error, warn};
use cdda_json_files::{
...
#[must_use]
pub fn shown_recipes(
    location: &LocationCache,
    infos: &Infos,
    sav: &Sav,
    hierarchy: &ItemHierarchy,
    crafter_pos: Pos,
    crafter_body_containers: &BodyContainers,
    ...
    let nearby_items = find_nearby(location, items, crafter_pos, crafter_body_containers);
    let nearby_pseudo_items = find_nearby_pseudo(location, infrastructure, crafter_pos);
    let nearby_sources = find_sources(location, infrastructure, crafter_pos);
