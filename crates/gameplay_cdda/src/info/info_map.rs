use crate::info::fixed::{human, wiring};
use crate::{Error, TypeId, info::parsed_json::Enriched};
...
impl<T: fmt::Debug + DeserializeOwned + 'static> InfoMap<T> {
    ...
    /// # Errors
    ///
    /// This function will return an error if the given `id` is unknown.
    pub fn get(&self, id: &InfoId<T>) -> Result<&Arc<T>, Error> {
        self.map
            .get(id)
