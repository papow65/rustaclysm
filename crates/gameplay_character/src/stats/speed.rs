use crate::{Breath, ChangePace, StaminaCost};
...
impl BaseSpeed {
    ...
    pub const fn from_kmph(s: f32) -> Self {
        Self(Speed::from_kmph(s))
    }

    pub const fn speed(self, walking_mode: &WalkingMode, breath: Breath) -> Speed {
...
impl WalkingMode {
    ...
    #[must_use]
    pub const fn stamina_impact(&self, breath: Breath) -> StaminaCost {
        match breath {
            Breath::Normal | Breath::AlmostWinded => match self {
    ...
    #[must_use]
    pub const fn switch(&self, change_pace: ChangePace) -> Self {
        match change_pace {
    ...
    #[must_use]
    pub fn breath_color(&self) -> TextColor {
        match self {
